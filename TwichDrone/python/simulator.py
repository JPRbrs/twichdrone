...
class MotorWidget:
    ...
    def Update(self, MR, ML):
        ...
        # angular data
        self.angular_a = ((math.fabs(Mo) * model.MotorModel.MAXSPEED)
                          / model.MotorModel.MAXPOWER)
        self.angular_d = 1  # left
        if Mo < 0:
            self.angular_d = -1  # right
        self.steering_angle = self.angular_d * (
            self.angular_a * model.DroneModel.MAX_STEERINGANGLE
        ) / model.MotorModel.MAXANGSPEED

        # linear data
        LF = (self.rmpower * self.rmforward) + (self.lmpower * self.lmforward)
        self.linear_a = (
            LF * model.MotorModel.MAXSPEED
        ) / model.MotorModel.MAXPOWER
    ...
    def Draw(self,
        ...
        pygame.draw.rect(
            TB.background,
            mlcolor,
            Rect((self.lmpos[0]-20, self.lmpos[1]-40), (40, 80)),
            1)
        pygame.draw.rect(TB.background,
                         mrcolor,
                         Rect((self.rmpos[0]-20, self.lmpos[1]-40), (40, 80)),
                         1)
        pygame.draw.rect(TB.background,
                         ccolor,
                         Rect((self.center[0]-50, self.center[1]-50),
                              (100, 100)),
                         1)
        ...
        pygame.draw.rect(TB.background,
                         ccolor,
                         Rect((self.lmpos[0]+20, self.center[1]-15), (30, 30)),
                         0)
        pygame.draw.rect(TB.background,
                         ccolor,
                         Rect((self.rmpos[0]-50, self.center[1]-15), (30, 30)),
                         0)
        ...
        pygame.draw.polygon(rect_surface,
                            Color("red3"),
                            ((0, self.attitude_ind[1]),
                             (self.attitude_ind[0]/2, 0),
                             (self.attitude_ind[0],
                              self.attitude_ind[1])),
                            0)

        oldRect = rect_surface.get_rect(center=(self.center))
        rect_surface, newRect = TB.rot_center(rect_surface, oldRect,
                                              (math.degrees(
                                                  self.steering_angle)))
        ...
        lend = (self.lmpos[0], self.lmpos[1] + (
            s_lmpower * self.lmforward) * -1)
        rend = (self.rmpos[0], self.rmpos[1] + (
            s_rmpower * self.rmforward) * -1)

        pygame.draw.line(TB.background,
                         lcolor,
                         self.lmpos,
                         lend,
                         3)  # *-1 Y axis are inverted
        pygame.draw.line(TB.background, rcolor, self.rmpos, rend, 3)

        if self.lmpos[1] < lend[1]:  # down arrow
            pygame.draw.polygon(TB.background,
                                lcolor,
                                ((lend[0], lend[1] + 3),
                                 (lend[0] - 3, lend[1] - 2),
                                 (lend[0] + 3, lend[1] - 2)), 0)
        ...
        TB.RenderText(14, "RIGHT_M", (self.rmpos[0], self.center[1]-50),
                      tcolor)
        TB.RenderText(14, "Front", (self.center[0], self.center[1]-40), tcolor)

        TB.RenderText(14, "%3.2f %c" % (self.lmpower, lf),
                      (self.lmpos[0]-50, self.center[1]),
                      tcolor)
        TB.RenderText(14, "%3.2f %c" % (self.rmpower, rf),
                      (self.rmpos[0]+50, self.center[1]),
                      tcolor)
        ...
        TB.RenderText(14,
                      "v: %3.2f*%d|%3.2fdeg" % (
                          self.angular_a,
                          self.angular_d,
                          math.degrees(self.steering_angle)*self.angular_d
                      ), (self.center[0], self.center[1]+40), tcolor)
...
class ArenaWidget:
    def __init__(self, position, width, height, caption, tdelta):
        ...
        self.droneRect = Rect(self.pos[0]+20,
                              self.pos[1]+20,
                              self.width-40,
                              self.height-50)
        self.drone = DroneSprite(self.center,
                                 tdelta)
    ...
    def Draw(self, TB, motors, tdelta, ccolor=Color("black"),
             tcolor=Color("black")):
        # arena
        pygame.draw.rect(TB.background,
                         ccolor,
        ...
        TB.RenderText(18, self.caption,
                      (self.pos[0]+self.width/2, self.pos[1]+10), tcolor)
        ...
        rect_surface, newRect = TB.rot_center(
            self.drone.image,
            oldRect,
            (math.degrees(self.drone.angle)) % 360)
        TB.background.blit(rect_surface, newRect)
...
if __name__ == "__main__":
    ...
    motors = MotorWidget((200, 50), TB.Width()-400, 150,
                         "Drone Motors (Power Range: 0-255)")
    arena = ArenaWidget((10, 210), TB.Width()-20, TB.Height()-220,
                        "Drone Simulator (Max Speed Motor: 0.6 m/s) " +
                        "[Click in Drone to RESET POS]", ltime)

    modeljoy.SetAngleRanges(model.DroneModel.FORWARD_ANGLE_RANGE,
                            model.DroneModel.BACKWARD_ANGLE_RANGE)
    emujoy.SetAngleRanges(model.DroneModel.FORWARD_ANGLE_RANGE,
                          model.DroneModel.BACKWARD_ANGLE_RANGE)
    ...
    while 1:
        ...
        TB.RenderText(18,
                      "[MODEL] Buttons: %s" % (
                          model.MODEL.getbuttons_str()),
                      (TB.Width()/2, 30), Color("black"))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONUP and\
               recbutton.MouseInside(mouse_x, mouse_y):
                    data_model = {
                        'kind': 'button',
                        'id': 'rec',
                        'pressed': 'true'
            ...
            if event.type == pygame.MOUSEBUTTONUP and\
               arena.drone.rect.collidepoint(mouse_x, mouse_y):
