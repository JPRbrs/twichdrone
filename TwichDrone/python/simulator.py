...
import wsock
import model
# The following three lines are the only difference with the simconsole file
# from operator import pos
# from __builtin__ import True
# from _curses import A_STANDOUT
...
class PyGameToolBox:
    ...
    def RenderText(self, fontsize, text, pos, color=Color("black"), surface=None):

        if not surface: surface = self.background
    ...
    def Width(self): return self.winsize[0]
    def Height(self): return self.winsize[1]
...
class JoyWidget:
    ...
    def Draw(self, TB, ccolor=Color("orange"),  tcolor=Color("black")):
        ...
        TB.RenderText(18, self.caption, (self.pos[0],self.pos[1]-self.radius-25), tcolor)

        #pygame.gfxdraw.pie( TB.background, Color("blue"), Rect(self.pos[0]-self.radius, self.pos[1]-self.radius,self.radius*2,self.radius*2), math.radians(self.forward_angle_range[0]),math.radians(self.forward_angle_range[1]))
        pygame.gfxdraw.pie( TB.background, self.pos[0], self.pos[1], self.radius, 360 - self.forward_angle_range[1],360 - self.forward_angle_range[0], Color("darkgreen") )
        pygame.gfxdraw.pie( TB.background, self.pos[0], self.pos[1], self.radius, 360 - self.backward_angle_range[1],360 - self.backward_angle_range[0], Color("purple") )
    ...
    def UpdateFromInput(self, TB, x, y , lcolor=Color("red")):
        ...
        distance = math.sqrt( math.pow(x-self.pos[0],2) + math.pow(y-self.pos[1],2) )

        pygame.draw.line(TB.background, lcolor , (self.pos[0], self.pos[1]), (x,y), 3)
        TB.RenderText(18,"DIST: %3.2f ANG: %3.2f deg" %  (distance, angle), (self.pos[0],self.pos[1]- self.radius-10), Color("black"))

        data_model =  {'kind': 'joystick', 'distance': distance, 'angle': math.radians(angle)  }
        return data_model
    ...
    def UpdateFromData(self, TB, data, lcolor=Color("red")):
        ...
        TB.RenderText(18,"DIST: %3.2f ANG: %3.2f deg" % (data.distance, math.degrees(data.angle)), (self.pos[0],self.pos[1]- self.radius-10), Color("black"))
        TB.RenderText(18,"DIST (Linear): %3.2f" % (data.distance_linear), (self.pos[0],self.pos[1]+ self.radius+10), Color("black"))
...
class ButtonWidget:
    ...
    def Draw(self, TB, ccolor=Color("red"),  tcolor=Color("black")):
        pygame.draw.rect (TB.background, ccolor , Rect(self.pos, (self.width,self.height)))
        TB.RenderText(14, self.caption, (self.pos[0]+self.width/2,self.pos[1]+self.height/2), tcolor)
...
class MotorWidget:
    def __init__(self, position, width, height, caption):
        ...
        self.center = (self.pos[0]+(self.width/2), self.pos[1]+self.height/2+10)
        ...
        self.angular_a = 0.0
        self.angular_d = 1   # 1: left, counterclockwise, incremental, -1: right, clockwise, decremental
    ...
    def Update(self, MR, ML):
        ...
        self.rmforward = 1
        if MR.direction == model.MotorModel.BACKWARD: self.rmforward = -1

        self.lmpower = ML.power
        self.lmforward = 1
        if ML.direction == model.MotorModel.BACKWARD: self.lmforward = -1
