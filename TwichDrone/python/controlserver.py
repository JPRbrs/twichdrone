...
if __name__ == "__main__":
    ...
    parser.add_argument("-v", "--verbose", help="Show detailed model info",
                        action="count", default=0)
    parser.add_argument("-a", "--address", help="Host Address", default='')
    parser.add_argument("-p", "--port", help="Listen Port", default=8000)
    parser.add_argument("-f", "--frequency", help="FPS to run model",
                        default=60)
    parser.add_argument("-g", "--gopropasswd", help="GoPro Password",
                        default='')
    parser.add_argument("-s", "--serialport", help="arduino serial port",
                        default='')
    ...
    print "ControlServer started at %s:%s (verbose:%s) Refresh: %s Hz" % (
    ...
    print "Gopro Status: Powered: %s [battery: %s %%]" % (
        gopro_status['power'], gopro_status['batt1']
    )
    ...
    while True:
        ...
        if olddata and (olddata['data'] != data['data'] or
                        olddata['MR'] != data['MR'] or
                        olddata['ML'] != data['ML'] or
                        olddata['buttons'] != data['buttons']):
            ...
            driver.DriveMotor(ArduMotor.MOTOR_LEFT,
                              data['ML'].direction,
                              data['ML'].power)
            driver.DriveMotor(ArduMotor.MOTOR_RIGHT,
                              data['MR'].direction,
                              data['MR'].power)
            # until here
