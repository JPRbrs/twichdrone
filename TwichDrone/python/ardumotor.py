...
class MotorPINS:
    ...
    def setup(self, arduino):
        arduino.pinMode(self.direction,
                        arduino.OUTPUT)
        arduino.pinMode(self.brake,
                        arduino.OUTPUT)
...
class ArduMotor:
    ...
    def __init__(self, port=None, verbose=0):
        ...
        try:
            ...
            print("Problem starting arduino interface: [skipping]")
            self.arduino = None
        self.motor_left = MotorPINS(label="LEFT",
                                    channel='B',
                                    direction=13,
                                    power=11,
                                    brake=8,
                                    current=1,
                                    default=ArduMotor.DEFAULT_LEFT)
        self.motor_right = MotorPINS(label="RIGHT",
                                     channel='A',
                                     direction=12,
                                     power=3,
                                     brake=9,
                                     current=0,
                                     default=ArduMotor.DEFAULT_RIGHT)

        self.motors = {ArduMotor.MOTOR_LEFT: self.motor_left,
                       ArduMotor.MOTOR_RIGHT: self.motor_right}
    ...
    def GetCurrent(self, motor):
        ...
        #print "Sensing %s:%s: %3.2f" % (m.label, m.channel, currentAmps)  ## NOQA
        return currentAmps
    ...
    def DriveMotor(self, motor, direction=None, power=None, brake=False):
        ...
        # sanity check
        if direction is None or power is None:
            return
        ...
        #print "Driving motor: %s:%s to %s: [power: %s] (brake: %s) " % (m.label, m.channel, ar_dir, power, brake)  ## NOQA
