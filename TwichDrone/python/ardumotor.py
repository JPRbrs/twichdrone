...
class MotorPINS:
    def __init__(self, label, channel, direction, power, brake, current,
                 default):
        self.label = label
...
class ArduMotor:
    ...
    def __init__(self, port=None, verbose=0):
        ...
        try:
            ...
            print "Error starting arduino: %s [skipping arduino interface]" % e
            self.arduino = None

        self.motor_left = MotorPINS("LEFT", 'B', 13, 11, 8, 1,
                                    ArduMotor.DEFAULT_LEFT)  # Channel B
        self.motor_right = MotorPINS("RIGHT", 'A', 12, 3, 9, 0,
                                     ArduMotor.DEFAULT_RIGHT)  # Channel A

        self.motors = {
            ArduMotor.MOTOR_LEFT: self.motor_left,
    ...
    def DriveMotor(self, motor, direction=None, power=None, brake=False):
        ...
        print "Driving motor: %s:%s to %s: [power: %s] (brake: %s) " % (
            m.label,
            m.channel,
            ar_dir,
            power,
